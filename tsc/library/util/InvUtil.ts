import { Container, Entity, EntityInventoryComponent, ItemStack, Player } from "@minecraft/server";
/**
 * 인벤토리 관련 클래스
 */
...
export class PInvUtil extends InvUtil {
     ...
     * @param item 셋할 아이템
     */
    setMainhand(item: ItemStack): void {
     ...
     * @returns 전체 아이템 []
     */
    getAllItem(): ItemStack[] {
        let Items: ItemStack[] = [];
        for (let i=0; i<36; i++) {
            const get = this.getItem(i)
            if(get) Items.push(get)
        }
        return Items;
    }
}