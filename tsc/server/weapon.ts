import { world, EntityDamageCause, EntityHurtAfterEvent, 
    system, Player, 
    ItemLockMode, ItemStack, EntityHealthComponent, EntityTypes } from "@minecraft/server";
...
system.runInterval(()=>{
    world.getAllPlayers().forEach(pl=>{
        ...
        const attackCoolData = AttackCoolDB.get(pl.id) as number;
        const selectedSlotData = SelectedSlotDB.get(pl.id) as number;
        ...
        if(attackCoolData>0) AttackCoolDB.set(pl.id, attackCoolData - 1);
...
world.afterEvents.entityHurt.subscribe((e)=>{
    ...
    if(damagingEntity?.typeId !== Player.prototype.typeId||e.damageSource.cause!==EntityDamageCause.entityAttack) return;

    const pInv = new PInvUtil(damagingEntity as Player);
    const itemStack = pInv.getMainhand();
    const attackCoolData = AttackCoolDB.get(damagingEntity.id) as number;
...
function returnCorrectTool(itemStack: ItemStack): tool {
    ...
    if(itemStack.hasTag("is_hoe")) return new hoe(itemStack);
};
...
class hoe implements tool {
    ...
    AfterHit(ev: EntityHurtAfterEvent): void {
        ev.hurtEntity.dimension.getEntities({"closest":5,"location":ev.hurtEntity.location,"maxDistance":4,"families":["mob"]}).forEach((en)=>{
            if(en == ev.damageSource.damagingEntity) return;
